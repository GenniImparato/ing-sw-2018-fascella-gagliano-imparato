...
public class CLILathekinView extends CLIView
{
    ...
    public void draw()
    {
        if(state == 0)
        {
            ...
            cli.showMessage("You are using a Lathekin tool card:");
        ...
        {
            cli.renderGameState(CLIRendererMainState.BOARD_SELECTED);
            cli.showMessage("Choose another die from your board:");
            cli.showMessage("b) go back");
            cli.notify(new CLIInputEvent(cli, this, cli.readInputFromUser()));
        }
    ...
    public void control(Game game, String input)
    {
        try
        {
            ...
            if(val >= 0 && val < 20) //checks if the input is in the range to represent a cell of the matrix
            {
                ...
                GameAction action;
                if(state == 0 || state == 2)
                    action = new SelectDieFromBoardAction(row, col);
                else
                    action = new UseToolCardAction(row, col);
